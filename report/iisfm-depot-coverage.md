...
| **Niryat** ([niryat.gov.in](https://niryat.gov.in)) — MoC&I export analytics portal | monthly export values by commodity group / district / destination | 🔴 unreachable from this machine | `ECONNREFUSED` on all three routes (direct, server-side fetcher, browser pane) — NIC 164.100.x.x host, same geo-fence pattern as e-NAM. Secondary limitation even when reachable: Niryat publishes **values only** (USD), no quantities — so it cannot produce unit values for price parity; DGCI&S/tradestat/APEDA remain the qty+value sources. Works from Indian IPs. |
